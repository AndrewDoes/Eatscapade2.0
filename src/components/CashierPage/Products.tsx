...
const Products = ({ products = initialProducts, onAddToCart }) => {
    return (
        <div className="h-[calc(100vh-100px)] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 oveflow-auto">
                {products.map(product => (
                    <div
                        key={product.id}
                        className="bg-white rounded-lg shadow-sm cursor-pointer hover:shadow-lg transition-shadow duration-200 border border-gray-200 flex flex-col"
